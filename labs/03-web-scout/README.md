...
- PAD + browser extension (แนะนำ baseline **2607+** — ดู [`shared/SOURCES-AUG2026.md`](../../shared/SOURCES-AUG2026.md))
...
1. สร้างโฟลเดอร์ `C:\PAD-Labs\output\lab03\` และ `C:\PAD-Labs\output\lab03\downloads\`
2. คัดลอก [`assets/upload-sample.txt`](assets/upload-sample.txt) ไปที่ที่เข้าถึงง่าย เช่น `C:\PAD-Labs\working\lab03\upload-sample.txt`
...
4. ลาก **Set variable**:
   ...
   - Value: `BKK`
5. ลาก **Create new data table** (หรือเทียบเท่าใน designer)
6. ตั้งคอลัมน์ให้ตรง template: `ScoutId`, `SourcePage`, `Key`, `Value`, `CapturedAt`, `Matched`, `Notes`
7. ชื่อ produced: `ScoutResults` ← **ไม่ใส่ `%`**  
   (อ้างอิงด้วย `%ScoutResults%`)

### Step 1 — เปิดเบราว์เซอร์

1. ลาก **Launch new Microsoft Edge** (หรือ **Launch new Chrome**)
2. Initial URL: `https://ontoiq.tech/pad/03-table.html` (เริ่ม Mission A)
...
1. ลาก **For each** → Value to iterate: `%StaticTable%` ← **ใช้** (มี `%`) → Store into: `StaticRow` ← **ไม่ใส่ `%`**
2. **ภายใน For each** ลาก **Insert row into data table** (หรือ Add row) เข้า `%ScoutResults%`
3. ใส่ค่าอย่างน้อย:
   - SourcePage = `03-table`
...
2. Browser instance: `%Browser%`
3. URL: `https://ontoiq.tech/pad/02-controls.html`
4. กด Save
...
1. ลาก **Insert row into data table** เข้า `%ScoutResults%`
2. ค่าตัวอย่าง:
   - SourcePage = `02-controls`
   - Key = `dropdown` (หรือชื่อ control)
...
2. URL: `https://ontoiq.tech/pad/09-ajax-table.html`
3. Browser: `%Browser%`
...
1. ลาก **For each** → Value to iterate: `%AjaxTable%` → Store into: `AjaxRow` ← **ไม่ใส่ `%`**
2. **ภายในลูป** ลาก **If** ตามเกณฑ์จาก `scout-criteria.csv` เช่น:
   - Amount >= `%MinAmount%` **หรือ**
   - Region ตรง `%TargetRegion%` (ถ้ามีคอลัมน์)
3. เมื่อเข้าเงื่อนไข: **Insert row into data table** เข้า `%ScoutResults%`
   - SourcePage = `09-ajax-table`
   - Notes = `PRIORITY HIT` เมื่อ Amount ≥ MinAmount (ตาม scout-brief)
   - Matched = `Yes` เมื่อผ่านเงื่อนไข
...
1. ลาก **Go to web page**
2. URL: `https://ontoiq.tech/pad/05-files.html`
3. Browser: `%Browser%`
...
2. บันทึกไฟล์ใต้ `C:\PAD-Labs\output\lab03\downloads\`  
...
2. Upload จาก `C:\PAD-Labs\working\lab03\upload-sample.txt` (หรือ path ที่คุณคัดลอกไว้)
3. Wait ผลลัพธ์บนหน้าถ้ามีข้อความยืนยัน

### Step D4 — บันทึก Scout row

1. **Insert row into data table** เข้า `%ScoutResults%`
2. ค่า:
   - SourcePage = `05-files`
   - Key = `DownloadOrUpload`
   - Value / Notes = path หรือผลลัพธ์
   - Matched = `Yes` เมื่อทำ download และ/หรือ upload สำเร็จ
...
1. **Go to web page** → `https://ontoiq.tech/pad/08-iframe.html`
...
5. อ่านค่า / บันทึกแถว Scout: SourcePage = `08-iframe`
6. กลับ parent frame ก่อนไปหน้าถัดไป

### Step F — API pulse (`12-api`) — เลือกทำ

1. **Go to web page** → `https://ontoiq.tech/pad/12-api.html` หรือใช้ **Invoke web service** ยิง GET health/orders ตาม URL บนหน้า
2. เก็บ HTTP status + snippet
3. บันทึกแถว Scout: SourcePage = `12-api`, Key = `health` (หรือ orders), Value = status เช่น `200`
...
1. ลาก **Go to web page**
2. URL: `https://ontoiq.tech/pad/19-catalog.html`
3. Browser: `%Browser%`
4. ลาก **Wait for web page content** รอ `#tbl-products`
...
2. Append / รวมแถวเข้า `%Products%` ← **ใช้** (มี `%`) (For each แล้ว Insert row หรือรวมตารางตามที่ designer รองรับ)

### Step P3 — Loop หน้าถัดไปตราบที่ Next ยังใช้ได้

1. ลาก **Loop condition** (หรือ Loop ตามเงื่อนไขที่ designer มี)
2. เงื่อนไขแนวคิด: ปุ่ม Next ยังไม่ disabled — ตรวจจาก UI element `#btn-next-page` / `[data-pad="page-next"]` หรือข้อความ `#lbl-page`
3. **ภายในลูป:**
...
- Mission G — Hover: https://ontoiq.tech/pad/13-hover.html เก็บ tooltip หลัง hover  
- Mission H — Popup: https://ontoiq.tech/pad/18-popup.html อ่านค่าจาก popup/new tab แล้วกลับแท็บหลัก
